...
infolder = './results/Archaea_clustered_cov80' #sys.argv[1]
...
def is_it_mixed(hits,outfolder):
    ...
    pool = ThreadPool(15)
    pool.imap_unordered(is_it_mixed, hits)
    
    for hit in hits:
        for i in tqdm(range(int(len(hits))), desc = "progress"):
...
def search_kingdom_vs_uniprot(uniprot_db, subgraphs, outfolder):
    ...
    if not os.path.isdir("{}/protein_information".format(outfolder)):
        ...
        print('direction: {}/protein_information made'.format(outfolder))
    else:
    ...
    for subgraph in subgraphs:
            ...
            json_file_path = "{}".format(subgraph)
            try:
                with open(json_file_path, 'r') as j:
                    target_ACCs = json.loads(j.read())
            ...
            if mostFrequent == 'no_protein_id':
                count = count + 1
    print('run finished'count)

    #individual function could be run for the max function
